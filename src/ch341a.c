...
struct libusb_device_handle *retDevHandle(void) {
    return devHandle;
};
...
int32_t ch341Configure(uint16_t vid, uint16_t pid)
{
    ...
    ssize_t cnt = libusb_get_device_list(devHandle, &list);
    if (cnt < 0) {
    ...
    for (ssize_t i = 0; i < cnt; i++)   {
        ...
        if ( ret == 0 )   {
            ...
            if ((iDesc.idVendor == vid) && (iDesc.idProduct == pid))    {
   ...
   /* if(!(devHandle = libusb_open_device_with_vid_pid(NULL, vid, pid))) {
    ...
    printf("This is a delay to check if it's possible to access another device with same vid pid.\n");
...
int32_t setDeviceHandler(struct libusb_device *functionDevice)    {

    struct libusb_device *device = functionDevice;
    uint8_t desc[0x12];
    int32_t ret;
    ...
    if(!(device = libusb_get_device(devHandle))) {
        fprintf(stderr, "Couldn't get bus number and address.\n");
        goto close_handle;
    }
...
{
    uint8_t out[CH341_MAX_PACKET_LEN];  //4096
    uint8_t in[CH341_PACKET_LENGTH]; //32
